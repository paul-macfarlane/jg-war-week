...
| discovered repository fact | MCP tool change | /api/mcp stays read-only and accepts a signed-in @jahnelgroup.com session or `Authorization: Bearer <MCP_TOKEN>` (ticket 21); never expose emails or the Organizer list or Host lists. |
