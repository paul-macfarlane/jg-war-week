...
- [x] **Reset the seed on staging.** Run the Seed workflow with `confirm_reset`. Expected: the stale `mvp` and `spirit` Awards are gone. Check: `/xi/awards` lists only the seeded Awards. (Moved from `.scratch/war-weeker/issues/29`.)
- [x] **Promote staging → main.** Open and merge the `staging` → `main` PR. Expected: production runs PRs #65–#71. Check: `pnpm smoke`-equivalent pages load on production: `/xi`, `/xi/leaderboard`, `/xi/finale`, `/api/mcp` with the token.
- [x] **Reset the seed on production** the same way, after promoting.
...
The last 3 items I'll do at a later date (written 9/26/26).
